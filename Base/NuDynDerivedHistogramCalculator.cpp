...
{
  if (reportStart("NuDynDerivedHistogramCalculator",getName(),"setDefaultConfiguration()"))
    {
    }
  ...
  if (reportDebug("NuDynDerivedHistogramCalculator",getName(),"setDefaultConfiguration()"))
    {
...
void NuDynDerivedHistogramCalculator::loadHistograms(TFile * inputFile)
{
  ...
  for (unsigned int iEventFilter=0; iEventFilter<nEventFilters; iEventFilter++ )
    ...
    for (unsigned int iParticleFilter=0; iParticleFilter<nParticleFilters; iParticleFilter++ )
      ...
      if (reportDebug("NuDynDerivedHistogramCalculator",getName(),"createHistograms()"))
        {
