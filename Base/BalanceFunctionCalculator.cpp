...
{
  if (reportStart("BalanceFunctionCalculator",getName(),"setDefaultConfiguration()"))
    ;
  ...
  if (reportDebug("BalanceFunctionCalculator",getName(),"setDefaultConfiguration()"))
    {
...
void BalanceFunctionCalculator::execute()
{
  ...
  outputFile->Close();
  if ( reportEnd("BalanceFunctionCalculator",getName(),"execute(...)"))
    { }
}
