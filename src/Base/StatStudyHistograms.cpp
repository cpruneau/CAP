...
void StatStudyHistograms::loadHistograms(TFile * inputFile)
{
  if (!inputFile)
    {
    if (reportError()) cout << "StatStudyHistograms::loadHistograms(...) inputFile is a null pointer" << endl;
    return;
    }
...
void StatStudyHistograms::saveHistograms(TFile * outputFile)
{
  if (!outputFile)
     {
     if (reportError()) cout << "StatStudyHistograms::loadHistograms(...) outputFile is a null pointer" << endl;
     return;
     }
  outputFile->cd();
