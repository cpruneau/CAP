...
//  int     targetPdgCode                  = 2212;
  ...
  switch (analysisParticleFilterOption)
    {
      ...
      case 4:  analysisParticleFilters = ParticleFilter::createChargedHadronFilters(YES,0.000, 20.0, YES,-6.0, 6.0, NO, 10.0, -10.0); break;
  ...
  if (runPairAnalysis)
    ...
    config.addParameter("min_eta",       -6.0); // 1;
    config.addParameter("max_eta",        6.0);  // 1;
    config.addParameter("nBins_phi",       72);
    config.addParameter("min_phi",        0.0);
    config.addParameter("max_phi",      TMath::TwoPi());
    config.addParameter("fillEta",      fillEta);
    ...
    switch (observableSelection)
      {
        ...
        sObservableNames.push_back("n1_eta");
        sObservableNames.push_back("n1_phi");
        pObservableNames.push_back("A2_DetaDphi_shft");
        pObservableNames.push_back("B2_DetaDphi_shft");
        pObservableNames.push_back("C2_DetaDphi_shft");
        pObservableNames.push_back("R2_DetaDphi_shft");
        break;
      }
